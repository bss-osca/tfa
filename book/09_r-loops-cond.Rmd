...
for(i in 1:nrow(distanceMat)) {
   for(j in 1:ncol(distanceMat)) {
      if (i>j) {__}   # assume symmetric distances
      if (!is.na(distanceMat[i,j])) next   # value already calculated 
      if (i==j) {__; next}
      __
   }
...
for(r in 1:nrow(ite)) {
   i <- ite$i[r]
   j <- ite$j[r]
   if (!is.na(distanceMat[j,i])) {distanceMat[i,j] <- distanceMat[j,i]; next}   # assume symmetric distances
   if (!is.na(distanceMat[i,j])) next   # value already calculated 
   if (i==j) {distanceMat[i,j] <- 0; next}
   distanceMat[i,j] <- getDist(i, j)
}
...
```
```{r, hint=TRUE, eval=FALSE}
ite <- expand_grid(i = __, j = __)
ite
distanceMat <- matrix(NA, nrow = length(idx), ncol = length(idx))
for(r in 1:nrow(ite)) {
   i <- ite$i[r]
   j <- __
   if (!is.na(distanceMat[j,i])) {__; next}   # assume symmetric distances
   if (!is.na(distanceMat[i,j])) next   # value already calculated 
   if (i==j) {distanceMat[i,j] <- 0; next}
   distanceMat[i,j] <- __
}
