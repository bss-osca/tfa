...
Sys.setenv(BING_MAPS_KEY=[your key]) # so you don't have to restart R
```

Add the line `BING_MAPS_KEY=[your key]` and save the file. 
   ...
   dat <- map_dfr(1:nrow(from_to_df), function(r) {
      url <- 
         str_c("http://dev.virtualearth.net/REST/V1/Routes/", mode, "?wp.0=", from_to_df$from[r], "&wp.1=", from_to_df$to[r], 
               ...
               "&avoid=minimizeTolls&key=", Sys.getenv("BING_MAPS_KEY"))
      url <- URLencode(url)
      lst <- fromJSON(url)
...
bing_geocode <- function(address) {
   dat <- map_dfr(address, function(s) {
      url <- str_c("http://dev.virtualearth.net/REST/v1/Locations?q=", 
                   s, "&key=", Sys.getenv("BING_MAPS_KEY"))
      url <- URLencode(url)
      lst <- fromJSON(url)
