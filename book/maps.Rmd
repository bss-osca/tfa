...
Often you need to connect to a service using an API for obtaining spatial data. The most common is Google and Bing (Microsoft) and to use the services you need an API key.
...
#'
#' @param dat The data frame returned from calling `calc_distances`.
...
```{r}
add_route <- function(dat = NULL, route, flow = 1, solution = 1) {
   route_id = if_else(is.null(dat), 1, max(dat$RouteId) + 1)
   tmp <- tibble(From = route[1:(length(route)-1)], To = route[2:length(route)])
   tmp <- tmp %>% mutate(Flow = flow, Sol = solution, RouteId = route_id)
   dat <- bind_rows(dat, tmp)
}

datLines <- add_route(route = c(2, 5, 1, 2)) %>% 
   add_route(route = c(2, 4, 3, 2), flow = 2) %>% 
   add_route(route = c(2, 3, 1, 2), flow = 2, solution = 2) %>% 
   add_route(route = c(2, 5, 4, 2), flow = 1, solution = 2)
...
A map with both solutions:

```{r}
datP <- datLines
m %>% addFlows(datP$FromLon, datP$FromLat, datP$ToLon, datP$ToLat,
               flow = datP$Sol, maxThickness = 2, color = col_pal[datP$Sol],
               opacity = 0.75, popup = popupArgs(labels = "Solution"))
```
