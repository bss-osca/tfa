...
The loop is used for running trough a set of objects (we will have a closer look at objects in Section \@ref(vba-objects)). Here the `rngC` is used to run throug all the cells in the range and set values. Note a range is scanned left-down.
...
If you want to separate both professors and associate professors from others, you may modify the if statement and use and `ElseIf`:
...
Consider column A in Figure \@ref(fig:sep-2). Assume we want to check if Jen is in a name and output her position. We can use a for loop for this:
...
We scan all rows for Jen and return her position. If we found her, then we exit the for loop (no need to search further). What happens if we search for `"Jen"` and not `"Jen "`? The same can be done using a while loop:
...
Assume that you have a set of $n$ locations: 
