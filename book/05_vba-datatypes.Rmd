...
Consider the Excel template file provided. The file contains different datasets in worksheets starting with _TM6_Data_. You may assume that all datasets have a header in the first row (starting in cell A1). Each column contains either doubles or strings. If the header title of a column starts with _str_ then the column contains strings; otherwise the column contains doubles. 

A dataset can be stored in two arrays where one store the column headers and one the data. For instance, the following code in VBA read the data in worksheet _TM6_Data1_ into two arrays:
...
Sub TM6_ReadData1()
   ...
   ThisWorkbook.Worksheets("TM6_Data1").Activate
...
(1) Given a header title, create a procedure `TM6_IsColumnString` with the following features:
...
(2) Given the array with header titles, create a procedure `TM6_GetColumnIndex` that return the column index of a header title with the following features:
...
(3) Create a procedure `TM6_SelectColumns` that selects a set of columns in a dataset stored using two arrays with the following features:
    ...
    - The modified dataset is output/pasted to the worksheet named _TM6_Select_ (starting in cell A1), i.e. no new arrays are needed to store the new dataset. This worksheet is cleared if it contains old data. 
    - The procedure should work for different datasets stored in the arrays (e.g. the dataset in worksheet _TM6_Data2_). 
    ...
    - Loads the dataset in worksheet _TM6_Data1_ in two arrays (a dataset of orders to a set of ships).
    ...
    Add a button running the procedure to worksheet _TM6_Select_.


(4) Create a procedure `TM6_Filter` that filter some rows based on a criteria in a dataset stored using two arrays with the following features:
    ...
    - The modified dataset is output to the worksheet named _TM6_Filter_ (starting in cell A1), i.e. no new arrays are needed to store the new dataset. This worksheet is cleared if it contains old data. 
    - If the header title or the compare operator is not valid then nothing should be output to the _TM6_Filter_ worksheet.
    ...
    Test `Filter` by finishing the code of procedure `TM6_FilterTest` that:
  
    - Loads the dataset in worksheet _TM6_Data1_ in two arrays.
    - Filter so get rows where price is greater than or equal 500000.
    - Create message box with the value of cell G2 in worksheet _TM6_Filter_ along with the column name.
    - Given the original dataset, filter so get rows where the item description contains _Gasket_.
    - Create message box with the value of cell B2 in worksheet _TM6_Filter_ along with the column name.
  
    Add a button running the procedure to worksheet _TM6_Filter_.


(5) The `TM6_Filter` procedure can only be used with a single compare operator. Consider the dataset in worksheet _TM6_Data1_ and discuss how a sequence of calls to `Filter` can be used to find rows where
