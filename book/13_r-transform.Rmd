...
<!-- Q4 -->
```{r, solution=TRUE, warning = FALSE, ignoreWarning = TRUE, text="The highest number of tests per capita in March 2021 was in Denmark."}
res1 <- dat %>%
...
### Exercise (election) {#ex-r-election}
...
```{r, solution=TRUE, text="We have a total of 99 municipalities."}
res <- dat %>%
  filter(year == 2017) %>% 
  distinct(area) %>% 
  nrow() %>% 
...
```{r, solution=TRUE, text="The highest number of votes was in 2017."}
res <- dat %>%
  group_by(year) %>% 
...
2) In which election year was the total number of votes highest?


```{r, solution=TRUE, text="The highest number of votes was given to the Social Democratic Party (1029326 votes) who got 842 elected."}
res <- dat %>%
  filter(year == 2017) %>% 
...
3) Consider the 2017 election. How many votes and elected candidates did the different parties get in total (sort your result descending by votes)?



```{r, solution=TRUE, text="The listed women is approx. 31. That is, listed women is lower than listed men. This percentage seems to be almost constant over the years (but increasing very slowly)."}
res <- dat %>%
  group_by(year) %>% 
...
res <- dat %>%
  group_by(___) %>% 
  summarize(listedWomen = ___, listedMen = ___) %>% 
  mutate(listedWomenPct = ___) %>% 
  arrange(listedWomenPct) %>% 
  print()
```

4) Calculate the percentage of females listed for each year. Are there on average more women than men listed? Is there any tendency over the years?



```{r, solution=TRUE, text="The Liberal Democratic Party got must relative votes in Herning."}
res <- dat %>%
  filter(year == 2017) %>% 
  group_by(area) %>% 
...
5) Consider the 2017 election. In which municipality did the Liberal Democratic Party get the highest percentage of votes? 





### Exercise (orders) {#ex-r-orders}
...
```{r, solution=TRUE, warning=FALSE, text="Orders are for 9 ships and from 31 suppliers. A total of 550 different items are considered."}
# load packages
library(tidyverse)
library(skimr)
skim(dat)
```
```{r, hint=TRUE, text="Use the `skim` package.", eval=FALSE}
```

1) How many ships, suppliers, and different items are considered?


```{r, solution=TRUE, text="I count the number of suppliers per order id. Since there is only a single supplier per order id, orders are sent from a single supplier as expected. Similar for delivery place."}
dat %>% 
...
2) Do all orders use a single supplier and delivery place? 


```{r, solution=TRUE, text="The highest delivery cost is 1585508 for order 356418604501."}
res <- dat %>% 
   group_by(order_id) %>% 
   summarise(
      items = n(), 
...
3) For each order, calculate/find the number of different items, number of items, total price, total delivery cost, and order year (missing values are assumed zero). Which order has the highest delivery cost?



```{r, solution=TRUE, text="The supplier most used is Supplier-447. The delivery place used most is Delivery Place-105."}
res1 <- dat %>% 
   group_by(order_id) %>% 
...
4) For each order, find the supplier and delivery place. Hint: The `first` function may be used to select the first item within a group. Which supplier and delivery place is used most?




5) Add a column to the dataset equal the *value* of the items in a row calculated as the price times the quantity. Next, create a summary table named `res1` that for each item id calculates the aggregated value and arrange them in descending order. 

```{r, solution=TRUE, text="The most costly item is 601.003.004 a Cylinder Liner IMO Number 1848265-3."}
res1 <- dat %>% 
...
res1 <- dat %>% 
   mutate(val = ___) %>% 
   group_by(___) %>% 
   summarise(val = ___, item_desc = first(___)) %>% 
   arrange(___) %>% 
   print()
```
...
res1 <- res1 %>% 
   ungroup() %>%
   mutate(pct = ___, cum_pct = cumsum(___)) %>%
   print() 
```
...
```{r, solution=TRUE, text="The item is used at 7 ships (most on ship Ship-12)."}
item <- res1$item_id[1]
res2 <- dat %>% 
   filter(item_id == item) %>% 
   group_by(ship) %>% 
...
```{r, solution=TRUE, warning=FALSE, text="It can be seen that a comma (`,`) is used a delimiter. That is, the dataset is loaded using `read_csv`."}
library(tidyverse)
lines <- read_lines(path, n_max = 3)
lines
dat <- read_csv(path)
```
```{r, hint=TRUE, text="Use the `read_lines` function to have a look at the csv file.", eval=FALSE}
```
...
```{r, solution=TRUE, warning=FALSE, text="We have a total of `r dat %>% nrow()` rows/observations for 9 ships over a period from 2015-01-24 to 2020-04-21."}
library(skimr)
skim(dat)
```
```{r, hint=TRUE, text="Use the `skimr` package.", eval=FALSE}
```

(2) Provide a short overview of the data. How many rows are there in the data, what is the number of different ships, and what is the range of dates?


```{r, solution=TRUE, text="We have a total of 463 jobs, the number of jobs for each ship can be seen using res2 with and average of 72.44 jobs per ship."}
res1 <- dat %>% 
   distinct(job_id) %>% 
   nrow() %>% 
   print()
...
(3) What is the total number of different jobs, number of different jobs per ship, and average number of jobs per ship? Hint: The function `n_distinct` may be used to find distinct values within a group.


```{r, solution=TRUE, text="The range and average of unique items considered can be seen using `res1` and the range and average of items used can be seen using `res2`. Note some jobs don't use the allocated item."}
res1 <- dat %>% 
...
(4) What is the minimum, average, and  maximum number of *different* items considered at each job? What is the minimum, average, and  maximum number of items used for maintenance at each job?


```{r, solution=TRUE, text="Ship-10 have a lot of jobs where the items scheduled in a job, have not been used."}
res <- dat %>% 
...
(5) It seems that some items considered in a job are not used anyway. Which ship has most jobs where an item should have been used, but has not (quantity is zero)?


```{r, solution=TRUE, text="Item 601.003.013 a Sealing Ring is used most"}
res <- dat %>% 
   group_by(item_id) %>%  
...
res <- dat %>% 
   group_by(___) %>%  
   summarize(n = ___, item_name = first(___)) %>% 
   arrange(___) %>% 
   print()
```

(6) Which item is the most used one for maintenance?
