...
This module gives a short introduction to VBA. So you can start programming fast and run your code.
...
Let us try to make your first piece of code. Download the template file `r link_excel_file(module_number_prefix, module_name)`, open the file and open the VBA editor under the **Developer** tab. 
   ...
   6) Go to the worksheet *`r sheet_name_prefix`*. 
...
The box can contain for instance a number, a date or a boolean. That is, any data type defined by VBA. Some of the basic data types used by VBA is:
...
More advanced data types such as a group of numbers (a numeric array), a range of cells in a worksheet (an object) or a set of numbers (a collection) will be considered in Module \@ref(vba-datatypes).
...
Variant  (a string)                       | 22 bytes + string length
...
Always declare your variables explicit in VBA. This is good coding practice since it reduces the memory requirements and type errors. You can add `Option Explicit` in the top of your module. This ensures that undefined variables raise an error. You can add it by default by modifying the preferences for the VBA editor (see Section \@ref(vba-setup)). You declare variables using the `Dim` keyword:
...
We will have a closer look on procedures in Module \@ref(vba-procedures).
...
You can also write/read values from a sheet using `Range` or `Cells`. Note it is always a good idea to know which sheet you are considering use the `Worksheets` function:
...
Sub TM2_ReadWriteSheet()
    ...
    int1 = Cells(9, 4)  ' row 9 and col 4
    ' Output values
    Range("B7") = str1 & "(" & int1 & ")"
    Cells(8, 2) = str1
End Sub
...
After declaring variables, the worksheet `r sheet_name_prefix` is activated and we input/output values using the `Range` and `Cells` functions. We will have a closer look on reading and writing data in Module ??.
...
After declaring variables and activating the worksheet, a dialog box is used for reading your name. Next, based on your answer we use an `If`/`Else` statement to do two different tasks. We will have a closer look on conditinal statements in Module ??.
