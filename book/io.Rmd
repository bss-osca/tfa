...
The first function puts all data in one column. The second function works even though we have some warnings.
...
info("FLYV41")
```
```{r, hint=TRUE, text = "You can modify the code in Question 3 to return only parts of the list.", title = "Hint 1"}
```
...
7) Modify `write_tuple` so it works if `shifts` also are given as input.
