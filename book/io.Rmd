...
## Different learning paths {#lp-io}
...
```
   ...
   4) Create a function `info(tab_id)` that returns a list with components `description`, `unit` and `variables` from the information for a data set with table id `tab_id`. Hint you can modify the code in Question 3 to return only parts of the list.
...
get_data <- function(tab_id, col_id = NULL) {
   ...
   url <- str_c("https://api.statbank.dk/v1/data/", lst$id[i], "/CSV?lang=en&", str_c(cols, collapse = "=*&"), "=*") %>% 
      URLencode()
   dat <- read_csv2(url)
   return(dat)
}
...
<!-- ### Exercise (Excel and Google Sheet) -->

<!-- ```{r} -->
<!-- gid <- "1U6Cf_qEOhiR9AZqTqS3mbMF3zt2db48ZP5v3rkrAEJY"   # gapminder data -->
<!-- ``` -->
