...
### Exercise - Pierre's bakery {#ex-vba-bakery}

Pierre owns a small bakery baking a single cold-rised bread. The demand level $l$ for bread is uncertain and on a given day the demand level equals $l=1$ (low) with probability 0.2, $l=2$ (medium) with probability 0.5 and $l=3$ (high) with probability 0.3. The actual demand (number of customers) depends on the demand level and is Poisson distributed with mean $20l$. The sales price per bread is 35 DKK and cost 14 DKK. If a customer arrives and Pierre has no bread left then he estimates the loss of goodwill to be 10 DKK. A customer always buy one bread. 

   (1) Create a function `Profit` that returns the daily profit given a specific demand and production.
   (2) Create a procedure `Simulation` that simulate the system for 500 days given that Pierre choose to produce $x$ breads. 
   (3) Create a procedure that prints statistics from `Simulation`. That is, the average, standard deviation and a plot with the profit distribution.  
   (4) Is it best for Pierre to produce 20 or 40 breads?
   (5) Create a function `OptimalProd` that search for the optimal production using the following steps:
       ...
       - Increase/decrease the production with one unit until no better average profit is found.
