...
* Add geometries to a plot such as a histogram, a scatter plot and a line.
...
[ggplot2][tidyverse-ggplot2] is a plotting package that makes it simple to create complex plots from data in a data frame. It provides an interface for specifying what variables to plot, how they are displayed, and general visual properties. Hence only minimal changes are needed if the underlying data change or if we decide to change from a bar plot to a scatterplot. 
...
[datacamp-r-ggplot2-intermediate]
...
## Different learning paths {#lp-r-plot}
...
__The west vs the developing world__
...
filter(gapminder, year == 1962) %>%
  ...
  geom_point() 
', text = "Most points fall into two distinct categories: 
...
', text = "This plot clearly shows that the majority of countries have moved from the _developing world_ cluster to the _western world_ one. In 2012, the western versus developing world view no longer makes sense. This is particularly clear when comparing Europe to Asia, the latter of which includes several countries that have made great improvements.")
``` 
```{r, echo=FALSE}
addSolution('
...
``` 
   ...
   2) In 1962, "the West versus developing world" view was grounded in some reality. Is this still the case 50 years later? 
   
      _Hint: we could easily plot the 2012 data in the same way we did for 1962. To make comparisons, side by side plots are preferable. In __ggplot2__, we can achieve this by _faceting_ variables and making a plot for each year. That is, you must filter by years 1962 and 2012 and add the layer `facet_grid`, which automatically separates the plots._
...
', text = "The plot clearly shows how most Asian countries have improved at a much faster rate than European ones.")
``` 
```{r, echo=FALSE}
addSolution('
gapminder %>% 
...
``` 
   ...
   3) To explore the transformation through the years, make a plot for years 1962, 1970, 1980, 1990, 2000 and 2012 considering Europe and Asia. How has Asia transformed through the years compared to Europe?
   
      _Hint: since we consider many years, we will not want all the plots on the same row. Instead, we will want to use multiple rows and columns. The function `facet_wrap` permits us to do this by automatically wrapping the series of plots._
      
  
<div class = "box">
__Observation - scales__
...
``` 
  ...
  4) Illustrate the transformation for Asia using a single plot where use year as color aesthetic. 



__Time series plots__
...
gapminder %>% filter(country %in% countries) %>% 
  ...
  geom_line() 
', text = "The plot clearly shows how South Korea's fertility rate dropped drastically during the 1960s and 1970s, and by 1990 had a similar rate to that of Germany.")
``` 
```{r, echo=FALSE}
addSolution('
...
gapminder %>% 
  ...
  geom_histogram(binwidth = 1, color = "black")
', text = "This provides a _close-up_ of the mid to lower income countries.")
``` 

  8) Make a histogram of `log2(dollars_per_day)` from 1970. 

<div class = "box">
__Observation - Which base?__ 
...
', text = "The plot from Q8 is the same except the values on the x-axis.")
``` 

  9) Make a histogram of `dollars_per_day` from 1970 using a log2 scale on the x-axis. Compare it to the plot from Question 8.
  
     _Hint: you can use the `scale_x_continuous` function with `trans = "log2"`._

The histograms in Question 8 and 9 have two _bumps_: one at about 4 and another at about 32. In statistics these bumps are sometimes referred to as _modes_. The mode of a distribution is the value with the highest frequency. The mode of the normal distribution is the average. When a distribution, like the one above, doesn't monotonically decrease from the mode, we call the locations where it goes up and down again _local modes_ and say that the distribution has _multiple modes_ indicating different distributions for different groups.
...
gapminder %>% 
  ...
  scale_x_continuous(trans = "log2")
', text = "The plot confirms the west vs the rest dichotomy.")
``` 
```{r, echo=FALSE}
addSolution('
gapminder %>% 
...
', text = "The income gap between rich and poor countries has narrowed considerably during the last 40 years")
``` 
```{r, echo=FALSE}
addSolution('
gapminder %>% 
...
gapminder %>% 
  ...
  geom_boxplot() +
  scale_y_continuous(trans = "log2") 
', text = "We now see that the rich countries have become a bit richer, but percentage-wise, the poor countries appear to have improved more. In particular, we see that the proportion of _developing_ countries earning more than $16 a day increased substantially.")
``` 
```{r, echo=FALSE}
addSolution('
gapminder %>% 
...
We used a series of histograms and boxplots to see this. Let us now shift to density plots. Let's start by noting that density plots for income distribution in `r past_year` and `r present_year` deliver the message that the gap is closing:
...
', text = "This plot now shows that the developing world distribution is changing.")
``` 
...
')
``` 
     e) Rotate the x-axis labels 90 degrees (try Googling "ggplot2 rotate axis labels").
```{r, echo=FALSE}
addSolution('
profit %>% 
  group_by(`Product Line`) %>% 
...
``` 
     ...
     Which product line is best and worst?
...
profit %>% 
  group_by(`Customer ID`) %>% 
  summarise(ctr = n(), t_profit = sum(Profit)) %>% 
  ggplot(aes(x = reorder(`Customer ID`, t_profit), y = ctr, fill = t_profit)) +
  geom_col() +
  labs(title = "Mean profit for each product line") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
``` 
  ...
  6) Make a plot illustrating number of transactions for each customer
...
profit %>% 
  group_by(`Customer ID`) %>% 
  mutate(t_profit_cust_id = sum(Profit),
         `Customer ID` = reorder(`Customer ID`, t_profit_cust_id)
         ) %>% 
  ggplot(aes(y = Profit, x = `Customer ID`)) +
  geom_boxplot() +
  # facet_wrap(facets = vars(reorder(`Customer ID`, Profit))) + 
  labs(title = "Mean profit for each product line") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
```
  ...
  2) Make a plot that shows the total profitability of the product lines in each quarter.
  
```{r}
profit %>% 
  group_by(Quarter, `Product Line`, Country) %>% 
  summarise(Profit = mean(Profit)) %>% 
  ggplot(aes(x = `Product Line`, y = Profit)) +
  geom_point() + 
  geom_col() + 
  facet_grid(cols = vars(Quarter), rows = vars(Country)) + 
  labs(title = "Mean profit for each product line") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
