...
The package [ggplot2][tidyverse-ggplot2] is a plotting package that makes it simple to create complex plots from data in a data frame. It provides an interface for specifying what variables to plot, how they are displayed, and general visual properties. Hence only minimal changes are needed if the underlying data change or if we decide to change from a bar plot to a scatterplot. 
...
## Combining plots into one using _patchwork_
...
```
```{r, hint=TRUE, title="Hint 1", text="Note you must aggegrate the numbers for the countries."}
dat %>% 
  group_by(date, ___) %>% 
...
```
```{r, hint=TRUE, title="Hint 1", text="First you have to filter type and country"}
dat %>% 
...
```{r, hint=TRUE, text="Use the `case_when()` function."}
```
...
```{r, hint=TRUE, text="You may need to consider quantity of purchases."}
```
...
```{r, hint=TRUE, text="*Hint:* You will need to consider quantity of purchases as well as price of lego sets."}
```
