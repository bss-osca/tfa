...
profit %>% 
  ...
  geom_col()
``` 
```{r, hint=TRUE, title="Hint 1", text="See the last section on this [webpage](https://sebastiansauer.github.io/ordering-bars/)."}
``` 
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
``` 
```{r, hint=TRUE, text="Try to google 'ggplot add title'."}
```
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
``` 
```{r, hint=TRUE, text="Try to google 'ggplot2 rotate axis labels'."}
```
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<!-- https://raw.githubusercontent.com/rstudio-education/datascience-box/master/course-materials/application-exercises/ae-01b-covid/covid.Rmd -->
...
# download.file("http://gapm.io/dl_pop", destfile = "test_pop1800_2100.xlsx")
world_pop = readxl::read_xlsx("test_pop1800_2100.xlsx", sheet = 7)
world_pop <- world_pop %>% 
  select(-geo, -indicator) %>% 
  rename(country = name) %>% 
...
dat %>%
  ggplot(aes(x = date, y = tot_cases, color = country, linetype = type)) +
  geom_line(size = 0.7, alpha = 0.8)

dat %>% filter(date >= now() - days(14), country == "Denmark", type == "confirmed") %>% 
  ggplot(aes(x = date, y = tot_cases, color = country, fill = type)) +
  geom_col(size = 0.7, alpha = 0.8)

dat %>% filter(date >= now() - days(21), type == "confirmed", !(country %in% c("US", "China"))) %>% 
  ggplot(aes(x = date, y = tot_cases, fill = country)) +
  geom_col(position = position_dodge2())


dat %>% 
  mutate(tot_cases_pop = tot_cases/pop) %>% 
  filter(date >= now() - days(21), type == "confirmed", !(country %in% c("US", "China"))) %>% 
