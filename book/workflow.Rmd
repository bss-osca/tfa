...
<!-- Exercises from: https://rafalab.github.io/dsbook/r-basics.html -->
...
Recall our car analogy from earlier. As we don't drive a car by interacting directly with the engine but rather by interacting with elements on the car's dashboard, we won't be using R directly but rather we will use RStudio's interface. After you install R and RStudio on your computer, you'll have two new *programs* (also called *applications*) you can open. We'll always work in RStudio (except when you do online courses) and not in the R application. Figure \@ref(fig:R-vs-RStudio-2) shows what icon you should be clicking on your computer. 
...
Now that you're set up with R and RStudio, you are probably asking yourself, "OK - now how do I use R?". The first thing to note is that unlike other software programs like Excel or SPSS that provide [point-and-click] interfaces, R is an [interpreted language][wiki-interpreted]. This means you have to type in commands written in *R code*. In other words, you have to code/program in R. Note that we'll use the terms "coding" and "programming" interchangeably in this book.
...
Must functions support the pipe operator which are a powerful tool for clearly expressing a sequence of multiple operations. The pipe operator `%>%`, comes from the magrittr package and is loaded automaticaly when you load tidyverse. You may use the RStudio keyboard shortcut Ctrl+Shift+M.
...
That is, we take what is left of the arrow (the object `x`) and input it into the function on the right of the arrow. These operations can be done using the pipe operator:
...
The above example is simple but illustrate that you can use pipes to skip intermediate assignment operations. Later you will do more complex pipes when we consider data wangling. For instance,

```{r}
mtcars %>% select(cyl, gear, hp, mpg) %>% filter(gear == 4, cyl == 4)
```

selects the columns related to cylinders, gears, hoursepower and miles, and then rows with cars having four cylinders and gears. For a more detailed introduction to pipes see [Chapter 18][r4ds-pipes] in @r4ds.
...
You now need a way to store R code in your project. We will use 2 ways of storing your code. A R script file or a R Markdown document. Normally you store lines of R code in a script file that you need to run. 
...
Change some things about your code. For instance set a sample size `n` at the top, e.g. `n <- 40`, and then replace all the hard-wired 40's with `n`. Change some other minor-but-detectable stuff, e.g. alter the sample size `n`, the slope of the line `b`,the color of the line etc. Practice the different ways to re-run the code:
...
Try to get an overview over the different planes and tabs. For instance in the Files tab (lower right plane) you can get an overview over your project files. You may also see this [video about projects][keyes-video-projects].
...
* Roger Peng has a lot of [videos](https://www.youtube.com/playlist?list=PLjTlxb-wKvXPqyY3FZDO8GqIaWuEDy-Od) on YouTube about R. Of special interest are
   ...
   - A introduction to the R language ([Part 1][video-r-intro-p1], [Part 2][video-r-intro-p2], [Part 3][video-r-intro-p3]) including data types and basic operations.
   - [A introduction to subsetting in R][video-subsetting].
...
Learn and use shortcuts as much as possible. For instance Alt+- for the assingment operator and Ctro+Shift+M for the pipe operator. A reference card of shortcuts can be seen using Alt+Shift+K.
...
<!-- [Try to add shortscuts etc for projects ...] -->

Exercises are numbered using [module number].[section number].[exercise number]. In general you are expected to start solving some of the exercises as homework (except the ones marked with an asterisk). When you meet in your study group, go through each exercise and discuss/solve it. 
...
You have all been allocated into groups. During the course you are expected to solve the R exercises in these groups. Before you start, it is a good idea to agree on a set of group rules:
   ...
   1) It is a good idea to have a common place for your code. Have a look at the section [Working in groups](#groups) and decide on a place to share your code.
   1) Create a common folder and project for your group.
   1) Agree on a coding convention.
   1) Agree about rules of how to meet etc.
...
bs_modal(id = "solution-i1", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="display:block;float:right") %>%
  bs_attach_modal(id_modal = "solution-i1")
```
 
   
You now have install the package from the default repository ([CRAN][cran]). 
...
<!--    2) Install the package `tfa` from github using path ``Tools4Analytics/course/ -->
...
selects the columns related to cylinders, gears, hoursepower and miles, and then rows with cars having four cylinders and (operator `&`) gears. 
...
bs_modal(id = "solution-p1", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-p1")
```

1) Create a pipe that selects columns related to miles, hoursepower, transmission and gears.
...
bs_modal(id = "solution-p2", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-p2")
```
...
bs_modal(id = "solution-p3", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-p3")
```
...
bs_modal(id = "solution-p4", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-p4")
```

4) Create a pipe that filter the cars having miles less than 20 and 4 gears and selects columns related to weight and engine.
...
bs_modal(id = "solution-p5", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-p5")
```
...
1) Which folders and files have been created? You may have a look in the **Files** plane.
...
bs_modal(id = "solution-w2", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-w2")
```
...
bs_modal(id = "solution-w4", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-w4")
```
...
bs_modal(id = "solution-v1", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-v1")
```
...
bs_modal(id = "solution-v2", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-v2")
```
...
bs_modal(id = "solution-v5", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-v5")
```
...
bs_modal(id = "solution-m2", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-m2")
```
...
bs_modal(id = "solution-m3", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-m3")
```
...
bs_modal(id = "solution-m4", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-m4")
```
...
bs_modal(id = "solution-m5", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-m5")
```
...
bs_modal(id = "solution-m6", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-m6")
```
...
bs_modal(id = "solution-m7", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-m7")
```
...
bs_modal(id = "solution-m8", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-m8")
```
...
bs_modal(id = "solution-m9", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-m9")
```
...
bs_modal(id = "solution-m10", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-m10")
```
...
bs_modal(id = "solution-m11", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-m11")
```
...
bs_modal(id = "solution-df2", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-df2")
```
...
bs_modal(id = "solution-df3", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-df3")
```
...
bs_modal(id = "solution-df4", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-df4")
```
...
bs_modal(id = "solution-df5", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-df5")
```
...
class(y)
```')
cat('\nOne bracket returns a list. Two returns the class of the component.')
sink()
}
bs_modal(id = "solution-list1", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-list1")
```
...
bs_modal(id = "solution-list2", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-list2")
```
...
bs_modal(id = "solution-list3", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-list3")
```
...
bs_modal(id = "solution-list4", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-list4")
```
...
The stringr package in tidyverse provide many useful functions for string manipulation. We will consider a few. 
...
bs_modal(id = "solution-str1", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-str1")
```
...
bs_modal(id = "solution-str2", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-str2")
```
2) Define new string that replace `BA` with `Business Analytics` in `str2`
...
bs_modal(id = "solution-str3", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-str3")
```
...
bs_modal(id = "solution-str4", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-str4")
```
...
bs_modal(id = "solution-str5", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-str5")
```
...
bs_modal(id = "solution-str6", title = "Solution", 
         body = includeMarkdown("tmp.md"))
bs_button("Solution", style="float:right") %>%
  bs_attach_modal(id_modal = "solution-str6")
```
