...
```{r detailed-course-proc, results='asis', echo=FALSE}
files <- c("ModRng", "ModAry", "ModWst", "ModCol", "ModTm", "ModRand")
...
for (f in files) {
   ...
   for (i in 1:length(str1)) {
      ...
      title <- str_extract(str, "\n'' (.*)", group = 1) %>% str_c("#### ", ., " {#def-vba-", funcName, "}\n")
      dat[[i]] = list(title = title, funcDef = funcDef, param = param)
   }
   tmp <- map(dat, out)
}
```
