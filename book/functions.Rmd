...
```{r}
test <- function() {
  # the function do some stuff and calculate some results
  res1 <- 45
  res2 <- "Success"
  res3 <- c(4, 7, 9)
...
The special argument `...` which indicate a variable number of arguments and is usually used to pass arguments to nested functions used inside the function. Consider example:
...
Scoping refers to the rules R uses to lookup the value of variables. A function will first look inside the body of the function to identify all the variables. If all variables exist no further search are required. Otherwise, R will look one level up to see if the variable exists. 
...
7) Given Question 5 the shipping cost can be split into parts. One part is gasoline which is 50% of the shipping cost. Write a function that has total cost a input argument and calculate the gasoline cost and uses the function defined in Question 5 inside it.   
...
8) Given Question 6 the shipping cost can be split into parts. One part is gasoline which is 50% of the shipping cost. Write a function that has total cost a input argument and calculate the gasoline cost and uses the function defined in Question 6 inside it. Hint: Use the `...` argument to pass arguments to `shipping_cost`.
...
```{r, echo=FALSE}
addSolution(text = "That value is still 3 since `x` defined inside the function is a local variable.")
```
...
```{r, echo=FALSE}
addSolution(text = "The code runs. But it is not good coding practice to call global variables inside a function (`x`). Instead `x` should have been an argument to the function.")
```

2) Any problems with the following code?
...
```{r, echo=FALSE}
addSolution(text = "That value is still 3 since `my_func` has not been called yet.")
```
...
```{r, echo=FALSE}
addSolution(text = "That value of `x` is 5 since `<<-` is used to look at the parent environment. The function call returns 11 since the `x` used is the local variable. In general avoid using `<<-` and give local variables different names compared to global ones.")
```
...
This exercise is based on on Exercise 6.12 in @vba-book. 
