...
add_footer <- function(module_number_prefix, module_name, use_index = F, use_excel = F) {
   slide_file <- str_c(module_number_prefix, "_", module_name, "-slides.html")
   if (use_index) {
      mod_file <- "https://bss-osca.github.io/tfa/index.html"
   } else {
      mod_file = str_c("https://bss-osca.github.io/tfa/mod-", module_number_prefix, "_", module_name)
   }
   if (use_excel) {
      excel_file <- str_c("https://github.com/bss-osca/tfa/blob/master/vba/", module_number_prefix, "-", module_name, "-template.xlsm")
      return(
         withTags({
            div(class="my-footer",
               span(
