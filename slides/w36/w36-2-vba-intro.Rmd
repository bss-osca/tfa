...
- Start keywords (such as `Sub` and `If`) with a capital letter (the VBA editor often correct this automatically for you).
- Only use letters a-z and A-Z in you names (some also use `_` but we will try to avoid it). That is don't use space in your names!
- Use names of procedures (i.e. subs or functions) that start with a capital letter.
